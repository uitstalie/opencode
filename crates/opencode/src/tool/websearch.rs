...
impl Tool for WebSearchTool {
    ...
    async fn execute(&self, p: ToolParams, _ctx: &ToolContext) -> ToolResult {
        ...
        if !resp.status().is_success() { return ToolResult::error(format!("HTTP {}", resp.status())); }
        let html = try_tool!(resp.text().await, |e| format!("Read: {}", e));

        let results = parse_results(&html, limit);
        if results.is_empty() { return ToolResult::text("No results found."); }
        let out: Vec<String> = results.iter().enumerate()
...
struct Sr { title: String, url: String, snippet: String }

fn parse_results(html: &str, limit: usize) -> Vec<Sr> {
    let mut results = Vec::new();
    for part in html.split("result__body").skip(1).take(limit) {
        let title = extract(part, "result__a\"", "</a>").or_else(|| extract(part, "result__title\"", "</a>")).unwrap_or("");
        let title = decode(strip_tags(title).trim());
        let url = extract(part, "result__url\"", "</a>").or_else(|| extract(part, "href=\"", "\"")).unwrap_or("");
        let url = decode(strip_tags(url).trim());
        let snippet = extract(part, "result__snippet\"", "</a>").or_else(|| extract(part, "result__snippet\">", "</")).unwrap_or("");
        let snippet = decode(strip_tags(snippet).trim());
        if !title.is_empty() && !url.is_empty() { results.push(Sr { title, url, snippet }); }
    }
    results
}
...
fn strip_tags(s: &str) -> String {
    ...
    for ch in s.chars() { if ch == '<' { in_tag = true; continue; } if ch == '>' { in_tag = false; continue; } if !in_tag { out.push(ch); } }
    out
}
...
mod tests {
    use super::*;
    #[test]
    fn strips() { assert_eq!(strip_tags("<b>Hi</b>"), "Hi"); }
    #[test]
    fn decodes() { assert_eq!(decode("&amp;&lt;"), "&<"); }
    #[test]
    fn extracts() { assert_eq!(extract(r#"<div class="title">Hello</div>"#, "title\">", "</div>"), Some("Hello")); }
}
