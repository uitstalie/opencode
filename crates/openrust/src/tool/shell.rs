...
pub fn shell_tool_prompt(kind: ShellKind) -> &'static str {
    match kind {
        ShellKind::Bash => r#"Use `bash` on Linux/macOS. Prefer concise shell commands.
...
- Keep commands safe and scoped to the requested task."#,
        ShellKind::Pwsh => r#"Use `pwsh` on modern Windows shells.
...
- Keep commands explicit and avoid shell-specific assumptions from Unix."#,
        ShellKind::Powershell => r#"Use `powershell` on legacy Windows PowerShell.
...
- Avoid Unix-only tools unless they are installed and explicitly available."#,
        ShellKind::Cmd => r#"Use `cmd` only when Windows shell compatibility is required.
