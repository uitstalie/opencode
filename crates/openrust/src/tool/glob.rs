...
impl Tool for GlobTool {
    ...
    async fn execute(&self, p: ToolParams, ctx: &ToolContext) -> ToolResult {
        ...
        let full = if pattern.starts_with('/') { pattern.to_string() }
                   else { format!("{}/{}", base.trim_end_matches('/'), pattern.trim_start_matches('/')) };

        let mut paths: Vec<String> = match glob::glob(&full) {
            Ok(iter) => iter.filter_map(|r| r.ok()).map(|p| p.to_string_lossy().to_string()).collect(),
            Err(e) => return ToolResult::error(format!("Invalid glob: {}", e)),
        };
        if paths.is_empty() { return ToolResult::text("No files matched."); }
        paths.sort();
        let total = paths.len();
        if total > 500 { paths.truncate(500); }
        let mut out = paths.join("\n");
        if total > 500 { out.push_str(&format!("\n... and {} more", total - 500)); }
        ToolResult::text(out)
    }
}
...
mod tests {
    ...
    async fn finds_rs_files() {
        ...
        let r = GlobTool.execute(ToolParams::new(serde_json::json!({
