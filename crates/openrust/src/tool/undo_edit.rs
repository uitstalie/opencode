...
impl Tool for UndoEditTool {
    fn name(&self) -> &'static str { "undo_edit" }
    ...
    async fn execute(&self, p: ToolParams, ctx: &ToolContext) -> ToolResult {
        ...
        let blob = match store.read_blob(hash) {
            Some(b) => b,
            None => return ToolResult::error(format!("Blob not found: {}. May have been GC'd (24h TTL).", hash)),
        };

        let new_hash = std::fs::read_to_string(path).ok()
        ...
        if let Some(h) = new_hash { msg.push_str(&format!("\nUndo hash: {}", h)); }
        ToolResult::text(msg)
    }
}
...
mod tests {
    ...
    async fn restores_content() {
        ...
        let r = UndoEditTool { undo_store: Some(s.clone()) }
            .execute(ToolParams::new(serde_json::json!({"filePath": tmp, "undoHash": hash})), &ctx(s)).await;
    ...
    async fn chaining() {
        ...
        let r = UndoEditTool { undo_store: Some(s.clone()) }
            .execute(ToolParams::new(serde_json::json!({"filePath": tmp, "undoHash": h1})), &ctx(s.clone())).await;
