...
impl Tool for WebFetchTool {
    ...
    fn description(&self) -> &'static str { "Fetch content from a URL, returning cleaned text." }
    ...
    async fn execute(&self, p: ToolParams, _ctx: &ToolContext) -> ToolResult {
        ...
        let out = if text.len() > 100_000 {
            format!("{}...\n\n[truncated at 100K / {} total]", &text[..100_000], text.len())
        } else { text };
        ToolResult::text(out)
    }
}
...
fn strip_html(html: &str) -> String {
    ...
    let mut in_tag = false; let mut in_script = false; let mut in_style = false;
    let lower = html.to_lowercase();
    for (i, ch) in html.char_indices() {
        if ch == '<' {
            let rest = &lower[i..];
            if rest.starts_with("<script") { in_script = true; }
            else if rest.starts_with("<style") { in_style = true; }
            else if rest.starts_with("</script") { in_script = false; }
            else if rest.starts_with("</style") { in_style = false; }
        ...
        if ch == '>' {
            ...
            if ctx.contains("</p") || ctx.contains("</div") || ctx.contains("</h") || ctx.contains("<br") {
                if !out.ends_with('\n') { out.push('\n'); }
            }
            continue;
        }
        ...
        if ch == '&' {
            let rest = &html[i..];
            if rest.starts_with("&amp;") { out.push('&'); continue; }
            if rest.starts_with("&lt;") { out.push('<'); continue; }
            if rest.starts_with("&gt;") { out.push('>'); continue; }
            if rest.starts_with("&quot;") { out.push('"'); continue; }
