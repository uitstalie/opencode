...
/// Trim history from the front so the total estimated tokens fit within
/// `context_window - output_reserve`. Always keeps at least `min_recent` messages.
/// Returns the number of messages trimmed.
pub fn trim_history(
    messages: &mut Vec<crate::core::provider::Message>,
    context_window: u64,
    output_reserve: u64,
    min_recent: usize,
) -> usize {
    let limit = context_window.saturating_sub(output_reserve) as usize;
    if limit == 0 || messages.len() <= min_recent {
        return 0;
    }
    ...
    for (i, msg) in messages.iter().enumerate().rev() {
        let size = token::estimate(msg.content.as_text());
        if total + size > limit {
            keep_from = (i + 1).min(messages.len());
            break;
        }
        total += size;
        keep_from = i;
    ...
    if keep_from < min_recent {
        keep_from = min_recent.min(messages.len());
    }
