...
## 高优先级
- [x] **完成 `/init` 实现**（commit `14dc67f7b`）：`SlashResult` 枚举 + `init_template()` 4-Phase 模板 + onboarding hint + slash help 补全
- [x] **push 未 push commits**：分支 `rust` 已 push 到 `origin/rust`（HEAD `f270c6514`）
- [x] **`detect_protocol` 去掉 fallback**：无 `protocol` 字段直接报错，已 commit + push + deploy
