...
function evalPerm(agent: Agent.Info | undefined, permission: string): PermissionV1.Action | undefined {
  ...
  return Permission.evaluate(permission, "*", undefined, undefined, agent.permission).action
}
...
it.instance("plan agent denies edits except .opencode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".opencode/plans/foo.md", undefined, undefined,  plan!.permission).action).toBe("allow")
  }),
)
...
it.instance("plan agent denies the general subagent by default", () =>
  Effect.gen(function* () {
    const plan = yield* load((svc) => svc.get("plan"))
    expect(plan).toBeDefined()
    expect(Permission.evaluate("task", "general", undefined, undefined,  plan!.permission).action).toBe("deny")
    expect(Permission.evaluate("task", "explore", undefined, undefined,  plan!.permission).action).toBe("allow")
    expect(Permission.evaluate("task", "custom", undefined, undefined,  plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const plan = yield* load((svc) => svc.get("plan"))
      expect(plan).toBeDefined()
      expect(Permission.evaluate("task", "general", undefined, undefined,  plan!.permission).action).toBe("allow")
    }),
  {
    config: {
      permission: {
...
it.instance("explore agent asks for external directories and allows whitelisted external paths", () =>
  Effect.gen(function* () {
    const explore = yield* load((svc) => svc.get("explore"))
    expect(explore).toBeDefined()
    expect(Permission.evaluate("external_directory", "/some/other/path", undefined, undefined,  explore!.permission).action).toBe("ask")
    expect(Permission.evaluate("external_directory", Truncate.GLOB, undefined, undefined, explore!.permission).action).toBe("allow")
    expect(
      Permission.evaluate("external_directory", path.join(Global.Path.tmp, "agent-work"), undefined, undefined, explore!.permission).action,
    ).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(Permission.evaluate("bash", "rm -rf *", undefined, undefined,  build!.permission).action).toBe("deny")
      // Edit still allowed
      expect(evalPerm(build, "edit")).toBe("allow")
    }),
  {
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"))
      expect(Permission.evaluate("external_directory", Truncate.GLOB, undefined, undefined, build!.permission).action).toBe("allow")
      expect(Permission.evaluate("external_directory", Truncate.DIR, undefined, undefined, build!.permission).action).toBe("deny")
      expect(Permission.evaluate("external_directory", "/some/other/path", undefined, undefined,  build!.permission).action).toBe("deny")
    }),
  {
    config: {
      permission: {
...
it.instance("global tmp directory children are allowed for external_directory", () =>
  Effect.gen(function* () {
    const build = yield* load((svc) => svc.get("build"))
    expect(
      Permission.evaluate("external_directory", path.join(Global.Path.tmp, "scratch"), undefined, undefined, build!.permission).action,
    ).toBe("allow")
    expect(Permission.evaluate("external_directory", "/some/other/path", undefined, undefined,  build!.permission).action).toBe("ask")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"))
      expect(Permission.evaluate("external_directory", Truncate.GLOB, undefined, undefined, build!.permission).action).toBe("deny")
      expect(Permission.evaluate("external_directory", Truncate.DIR, undefined, undefined, build!.permission).action).toBe("deny")
    }),
  {
    config: {
...
`,
      ...
      expect(Permission.evaluate("external_directory", target, undefined, undefined, build!.permission).action).toBe("allow")
    }),
  { git: true },
)
